...
function switchTab(tabName) {
  ...
  async function recognizeTextViaAPI(file) {
    ...
    // Nếu dịch được kích hoạt, lấy thêm ngôn ngữ đích
    ...
    try {
      ...
      if (response.ok) {
        // Trả về cả nội dung gốc và nội dung dịch, cách nhau bởi dấu xuống dòng
        return result.text + "\n\nTranslation:\n" + result.translation;
      } else {
  ...
  document.getElementById("exportBtn").addEventListener("click", async () => {
    ...
    try {
      ...
      // Xuất ra nhiều file Excel theo từng thư mục con
      for (const subFolderPath of Object.keys(folderMap)) {
        ...
        for (let i = 0; i < imageFiles.length; i++) {
          ...
          try {
            const recognitionPromise = enableOCR ? recognizeTextViaAPI(file) : Promise.resolve("");
            const [dimensionsData, base64Data, recognizedText] = await Promise.all([
